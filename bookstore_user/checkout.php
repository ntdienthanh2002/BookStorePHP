...
require_once './assets/data/order_data.php';
?>
﻿<!doctype html>
                            ...
                            <div class="checkout_form">
                                <div class="row">
                                    <div class="col-lg-6 col-md-6">
                                        ...
                                        <?php
                                        $uid = $_SESSION['uid'];
                                        $stmt = $conn->prepare("SELECT tknd_tendangnhap, tknd_sodienthoai, tknd_diachi FROM taikhoannguoidung WHERE tknd_ma = '$uid'");
                                        $stmt->execute();
                                        ...
                                        <div class="row">
                                            <div class="col-12 mb-30">
                                                ...
                                                <input type="text" name="uname" readonly="" value="<?php echo $result['tknd_tendangnhap'] ?>">     
                                            </div>
                                            <div class="col-12 mb-30">
                                                <label>Số điện thoại giao hàng <span>*</span></label>
                                            ...
                                            <div class="col-12 mb-30">
                                                ...
                                                <input type="date" name="deli">
                                                <?php
                                                if (isset($_POST['order'])) {
                                                    $note = trim($_POST['note']);
                                                    $deli = trim($_POST['deli']);
                                                    $today = date("Y-m-d");
                                                    if ($deli == "") {
                                                        ?>
                                                        <span>Vui lòng nhập ngày giao hàng.</span>
                                                        <?php
                                                    } else {
                                                        $least = strtotime(date("Y-m-d", strtotime($today)) . " + 2 day");
                                                        $least = strftime("%Y-%m-%d", $least);
                                                        if ($deli >= $least) {
                                                            $sql = "INSERT INTO donhang (tknd_ma, dh_ngaydathang, dh_ngaygiaohang, dh_diachigiaohang, dh_sdtgiaohang, dh_ghichu)"
                                                                    . "VALUES (?, ?, ?, ?, ?, ?)";
                                                            $stmt = $conn->prepare($sql);
                                                            @$stmt->bind_param("isssis", $_SESSION['uid'], $today, $deli, $addr, $phone, $note);
                                                            $stmt->execute();

                                                            $order = new Donhang("", "");
                                                            $result = $order->getOrderNew($conn);
                                                            ...
                                                            if (isset($_SESSION['cart'])) {
                                                                ...
                                                                $stmt = $conn->prepare("select sp_ma, sp_ten, sp_gia, sp_soluongtonkho, sp_giamgia from sanpham where sp_ma in ($strIds)");
                                                                $stmt->execute();
                                                                $result = $stmt->get_result();
                                                                while ($row = $result->fetch_assoc()) {
                                                                    $sql2 = "INSERT INTO chitietdonhang (dh_ma, sp_ma, ctdh_gia, ctdh_soluong, ctdh_giamgia)"
                                                                            . "VALUES (?, ?, ?, ?, ?)";
                                                                    $stmt2 = $conn->prepare($sql2);
                                                                    @$stmt2->bind_param("iiiii", $orderid, $row['sp_ma'], $row['sp_gia'], $_SESSION['cart'][$row['sp_ma']]['quantity'], $row['sp_giamgia']);
                                                                    $stmt2->execute();

                                                                    $sql3 = "UPDATE sanpham SET sp_soluongtonkho = sp_soluongtonkho - ? WHERE sp_ma = ?";
                                                                    $stmt3 = $conn->prepare($sql3);
                                                                    @$stmt3->bind_param("ii", $_SESSION['cart'][$row['sp_ma']]['quantity'], $row['sp_ma']);
                                                                    $stmt3->execute();
                                                                }
                                                            }
                                                            ?>
                                                            ...
                                                            ?>
                                                            <span>Ngày giao hàng phải sau ít nhất 2 ngày so với hiện tại.</span>
                                                            <?php
                                                        }
                                                    }
                                                }
                                            ...
                                            <table>
                                                ...
                                                <tbody>
                                                    ...
                                                    if (isset($_SESSION['cart'])) {
                                                        ...
                                                        while ($row = $result->fetch_assoc()) {
                                                            ?>
                                                            <tr>
                                                                <td><?php echo $row['sp_ten'] ?><strong> × <?php echo $_SESSION['cart'][$row['sp_ma']]['quantity'] ?></strong></td>
                                                                <?php
                                                                if ($row['sp_giamgia'] > 0) {
                                                                    ?>
                                                                    <td><?php echo $row['sp_giamgia'] ?> %</td>
                                                                <?php } else { ?>
                                                                    <td>-</td>
                                                                <?php } ?>
                                                                ...
                                                                <?php
                                                                if ($row['sp_giamgia'] > 0) {
                                                                    $total_price += ($row['sp_gia'] - ($row['sp_gia'] * ($row['sp_giamgia'] / 100))) * $_SESSION['cart'][$row['sp_ma']]['quantity'];
                                                                    ?>
                                                                    <td><?php echo ($row['sp_gia'] - ($row['sp_gia'] * ($row['sp_giamgia'] / 100))) * $_SESSION['cart'][$row['sp_ma']]['quantity'] ?> đ</td>
                                                                    <?php
                                                                } else {
                                                                    $total_price += $row['sp_gia'] * $_SESSION['cart'][$row['sp_ma']]['quantity'];
                                                                    ?>
                                                                    <td><?php echo $row['sp_gia'] * $_SESSION['cart'][$row['sp_ma']]['quantity'] ?> đ</td>
                                                                <?php } ?>
                                                            </tr>
                                                            <?php
                                                        }
                                                ...
                                                <tfoot>
                                                    <tr class="order_total">
                                                        <th>Tổng tiền</th>
                                                        <td></td>
                                                        <td><strong><?php echo $total_price ?> đ</strong></td>
                                                    </tr>
                                                </tfoot>
                                            </table>     
                                        </div>
